    ...
    def __init__(self, host : str = 'localhost', user : str = 'root', password : str = '', database : str = '') -> None:
        try:
            ...
            try:
                ...
                self.cursor.execute('CREATE TABLE patients (id INT NOT NULL AUTO_INCREMENT, rut INT NOT NULL, name VARCHAR(64) NOT NULL, admission DATE NOT NULL, prevision INT, afp INT, fkRUT INT, reason VARCHAR(128), derivation INT, box INT, hDays INT, PRIMARY KEY (id), FOREIGN KEY (fkRUT) REFERENCES workers(rut))')
                self.cursor.execute('CREATE TABLE remunerations (id INT NOT NULL AUTO_INCREMENT, fkRUT INT, brute INT, afp INT, prevision INT, plus INT, liquid INT, PRIMARY KEY (id), FOREIGN KEY (fkRUT) REFERENCES workers(rut))')
                self.cursor.execute('CREATE TABLE charges (id INT NOT NULL AUTO_INCREMENT, rut INT, brute INT, discount INT, extra INT, total INT, PRIMARY KEY (id))')
                self.connection.commit()
                print('Connected Succesfully!')
